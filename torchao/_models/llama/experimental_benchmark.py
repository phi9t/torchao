...
def main(
    ...
    # TODO: json dump the results

    repro_command: str = (
        f"python generate.py --quantization {quantization} --checkpoint_path {checkpoint_path} --device {device} --precision {precision} --compile {compile} --compile_prefill {compile_prefill} --profile {profile} --profile {memory_profile} --interactive --num_samples {num_samples} --max_new_tokens {max_new_tokens} --top_k {top_k} --temperature {temperature} --cache_size {cache_size} --kv_cache_quantization {kv_cache_quantization} --linear_causal_mask"
    )

    benchmark_results_path = pathlib.Path("autogen_benchmark_results")
    benchmark_results_path.mkdir(exist_ok=True, parents=True)
    ...
    if write_result:
        ...
        result_dict = {
            ...
            "quant": quantization,
            "mod": checkpoint_path.parent.name,
            "kv_quant": kv_cache_quantization,
            "compile": compile,
            "compile_prefill": compile_prefill,
            "dtype": str(precision),
            "device": device,
            "repro_command": json.dumps(repro_command),
            "max_new_tokens": max_new_tokens,
            "top_k": top_k,
            "temperature": temperature,
        }
        unique_id = f"{timestamp}_{checkpoint_path.parent.name}_{quantization}_{device}_{precision}"
        result_json_path = benchmark_results_path / f"{unique_id}.json"
        result_repro_command_path = (
            benchmark_results_path / f"{unique_id}_repro_command.sh"
        )
        result_repro_command_path.write_text(repro_command)
        logging.info(f"Writing results to {result_json_path}")
        rich.print(result_dict)
        with result_json_path.open("w") as fout:
