...
rich.pretty.install()
rich.traceback.install(show_locals=False, extra_lines=7)

PKG_BASE_PATH = pathlib.Path.home() / "torchao" / "torchao"
...
if str(wd) not in sys.path:
    logging.info(f"Adding {wd} to sys.path")
...
def prefill(
    ...
    """
    Prefill the model with a single token.
    This function is compiled (via `torch.compile`) to reduce overhead.
    """
    # input_pos: [B, S]
...
def decode_n_tokens(
    ...
    **sampling_kwargs,
    ...
    for _ in tqdm.trange(num_new_tokens):
...
def get_or_create(var_name: str, var_builder_fn: Callable[[], Any]):
    assert callable(var_builder_fn), "var_builder_fn must be a callable"
    # Get the caller's frame
    caller_frame = inspect.currentframe().f_back

    # Access the globals of the caller
    caller_globals = caller_frame.f_globals

    # Check if the variable exists in caller's globals
    if var_name not in caller_globals:
        # If not, create it with the default value
        caller_globals[var_name] = var_builder_fn()
        rich.print(f"Created global variable '{var_name}'")
    else:
        rich.print(f"Global variable '{var_name}' already exists")
...
def main(
    ...
    t0 = time.time()
    model: torch.nn.Module = get_or_create(
        "model", lambda: _load_model(checkpoint_path, device, precision)
    )
    # Transformer(
    ...
    rich.print(f"Time to load model: {time.time() - t0:.02f} seconds")
    ...
    if quantization:
        ...
        if "autoquant" in quantization:
            ...
            if not TORCH_VERSION_AT_LEAST_2_5:
                rich.print("Unwrapping tensor subclass since PyTorch version is < 2.5")
...
checkpoint_path = (
    PKG_BASE_PATH.parent / "checkpoints/meta-llama/Llama-2-7b-chat-hf/model.pth"
)
assert checkpoint_path.exists()
assert checkpoint_path.is_file()


# BEGIN: setup input parameters
prompt: str = "Hello, my name is"
interactive: bool = False
num_samples: int = 1
max_new_tokens: int = 100
...
precision = torch.bfloat16
write_result: Optional[Path] = None
# END: setup input parameters

# configurations for torch
torchao.quantization.utils.recommended_inductor_config_setter()
...
# Set this envvar to enable codegen logging
...
tokenizer = get_tokenizer(tokenizer_path, checkpoint_path)
# validate that tokenizer works
user_input_prompt = "What is your name?"
user_input_prompt_encoded = encode_tokens(
    tokenizer, user_input_prompt, bos=True, device=device
)
prompt_length = user_input_prompt_encoded.size(0)
rich.print(f"Prompt length: {prompt_length}")

# optionall run quantization
model = torchao.utils.unwrap_tensor_subclass(model)
model_size_gb = get_model_size_in_bytes(model, ignore_embeddings=True) / 1e9
rich.print(f"Model size: {model_size_gb:.02f} GB")
...
start = -1 if compile else 0
torch.cuda.reset_peak_memory_stats()
user_prompt: str = "Hello, my name is"
prompt: str = f"{B_INST} {user_prompt.strip()} {E_INST}"
encoded_prompt = encode_tokens(tokenizer, prompt, bos=True, device=device)
