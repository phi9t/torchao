...
def convert_hf_checkpoint(
    *,
    ...
    if not is_llama3:
        # TODO: show the checkpoint dir structure
        for path in checkpoint_dir.glob("**/*"):
            print(path)
        model_map_json = checkpoint_dir / "pytorch_model.bin.index.json"
        assert model_map_json.exists(), f"File {model_map_json} not found."
        assert model_map_json.is_file(), f"File {model_map_json} is not a file."
